...
#include "menu.h"
  ...
  void read_write_cfu(void) {
    ...
    int32_t ret = cfu_op0(9, addr, in_data4);  // Read global bufer A
    printf("Set Buffer A, in: %d, \t\taddr: %lX, \t\tout: %lX\n", in_data4,
           addr, ret);

    cfu_op0( 10, addr + 1000, in_data4 + 1000);  // Set global bufer B
    int32_t ret2 = cfu_op0( 11, addr + 1000, 256);  // Read global bufer B
  ...
  struct Menu MENU = {
      ...
      "project",
      {
          ...
          MENU_ITEM('n', "check block design of 4*4 with negtive",
                    block_matrix_4by4n),
          MENU_END,
      },
  };
